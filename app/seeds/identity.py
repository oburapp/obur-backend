...
Seed rows (venue categories, global product types) have slug-based stable
identity, not database-generated identity — the same slug must always
resolve to the same UUID so the seed migration can bulk-upsert
self-referencing (a category's `parent_id`) and cross-referencing (a
product type's `category_id`) rows in a single pass, with no
...
def global_product_type_id(slug: str) -> uuid.UUID:
    """Deterministic id for a GLOBAL_PRODUCT_TYPE row, derived from its slug."""
    return uuid.uuid5(_SEED_NAMESPACE, f"global_product_type:{slug}")
